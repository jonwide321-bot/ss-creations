...
const Testimonials: React.FC<TestimonialsProps> = ({ testimonials }) => {
  return (
    <section className="py-32 bg-[#FAF9F6] overflow-hidden relative">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[#D4AF37]/5 font-bold cinzel text-[30rem] select-none pointer-events-none rotate-12">
        LOVE
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-12">
          {testimonials.map((t, idx) => (
            ...
            >
              <div className="absolute top-10 right-10 text-[#D4AF37]/10 group-hover:text-[#D4AF37]/20 transition-all duration-500 transform group-hover:-rotate-12">
              ...
              <div className="flex items-center gap-5">
                <div className="relative">
                  <div className="absolute inset-0 bg-[#D4AF37] rounded-full scale-110 blur-sm opacity-0 group-hover:opacity-20 transition-opacity" />
                  <img src={t.avatar} alt={t.name} className="w-14 h-14 rounded-full object-cover relative z-10 border-2 border-[#FCFBF7]" />
                </div>
                <div>
                  <h4 className="font-bold text-slate-800 text-base uppercase tracking-widest">{t.name}</h4>
